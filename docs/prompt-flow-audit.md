# YouAreMemory Prompt Flow Audit
...
OpenClaw 会先把 workspace bootstrap 文件注入到宿主 system prompt 里的 Project Context。YouAreMemory 不接管这一步，也不会自动改写这些文件。
...
3. 生成一段 YouAreMemory runtime contract
4. 如果拿到了 `retrieved.context`，把证据块拼到 runtime contract 后面
5. 通过 `appendSystemContext` 注入给 OpenClaw

注意：代码当前是 `appendSystemContext`，不是 `prependContext` 或 `prependSystemContext`。
...
3. 我们插件追加的 YouAreMemory runtime contract
